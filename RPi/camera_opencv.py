...
class CVThread(threading.Thread):
    ...
    def findLineTest(self, posInput, setCenter):
        if not posInput:
    ...
    def findColor(self, frame_image):
        ...
        if len(cnts) > 0:
            ...
            X = int(self.box_x)
            Y = int(self.box_y)
            if Y < 240 - CVThread.tor: robot.lookUp()
            elif Y > 240 + CVThread.tor: robot.lookDown()
            if X < 320 - CVThread.tor: robot.lookLeft()
            elif X > 320 + CVThread.tor: robot.lookRight()
        else:
            self.findColorDetection = 0
        self.pause()
    ...
    def faceDetectCV(self, frame_image):
        ...
        self.faces = faceCascade.detectMultiScale(grayGen, scaleFactor=1.2, minNeighbors=5, minSize=(20, 20))
        if len(self.faces):
            robot.lightCtrl('red', 0)
            robot.buzzerCtrl(1, 0)
        else:
    ...
    def run(self):
        while 1:
            ...
            if self.CVMode == 'none':
                ...
                self.CVThreading = 1; self.findColor(self.imgCV); self.CVThreading = 0
            elif self.CVMode == 'findlineCV':
                self.CVThreading = 1; self.findlineCV(self.imgCV); self.CVThreading = 0
            elif self.CVMode == 'watchDog':
                self.CVThreading = 1; self.watchDog(self.imgCV); self.CVThreading = 0
            elif self.CVMode == 'faceDetection':
                self.CVThreading = 1; self.faceDetectCV(self.imgCV); self.CVThreading = 0
...
class Camera(BaseCamera):
    ...
    def colorFindSet(self, invarH, invarS, invarV):
        ...
        HUE_1, HUE_2 = invarH+15, invarH-15
        if HUE_1>180: HUE_1=180
        if HUE_2<0: HUE_2=0
        SAT_1, SAT_2 = invarS+150, invarS-150
        if SAT_1>255: SAT_1=255
        if SAT_2<0: SAT_2=0
        VAL_1, VAL_2 = invarV+150, invarV-150
        if VAL_1>255: VAL_1=255
        if VAL_2<0: VAL_2=0
        colorUpper = np.array([HUE_1, SAT_1, VAL_1])
        colorLower = np.array([HUE_2, SAT_2, VAL_2])

    def modeSet(self, invar): Camera.modeSelect = invar
    def upperIP(self, invar): global upperGlobalIP; upperGlobalIP = invar
    def linePosSet_1(self, invar): global linePos_1; linePos_1 = invar
    def linePosSet_2(self, invar): global linePos_2; linePos_2 = invar
    def colorSet(self, invar): global lineColorSet; lineColorSet = invar
    def randerSet(self, invar): global frameRender; frameRender = invar
    def errorSet(self, invar): global findLineError; findLineError = invar
...
def commandAct(act, inputA):
    global speedMove
    if act == 'forward': robot.forward(speedMove)
    elif act == 'backward': robot.backward(speedMove)
    elif act == 'left': robot.left(speedMove)
    elif act == 'right': robot.right(speedMove)
    elif act == 'DS': robot.stopFB()
    elif act == 'TS': robot.stopLR()
    elif 'wsB' in act:
        speedMove = int(act.split()[1])
        if 1 < speedMove <= 100: robot.speedSet(speedMove)
    elif act == 'up': robot.lookUp()
    elif act == 'down': robot.lookDown()
    elif act == 'UDstop': robot.lookStopUD()
    elif act == 'lookleft': robot.lookLeft()
    elif act == 'lookright': robot.lookRight()
    elif act == 'LRstop': robot.lookStopLR()
    elif act == 'jump': robot.jump()
    elif act == 'handshake': robot.handShake()
    elif act == 'steady': robot.steadyMode()
    elif act == 'steadyOff': robot.steadyMode()
    elif act == 'faceDetection': Camera.modeSelect = 'faceDetection'
    elif act == 'faceDetectionOff': Camera.modeSelect = 'none'
    elif 'trackLine' == act: Camera.modeSelect = 'findlineCV'; Camera.CVMode = 'run'
