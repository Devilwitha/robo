    ...
    while True:
        ...
        try:
            ...
            try:
                ...
                data = data_raw # BehÃ¤lt die Daten als String bei, falls es kein JSON ist

            if not data:
                continue

            print(f"DEBUG: Empfangener Befehl: {data}")
            
            # Special logging for speed commands
            if isinstance(data, str) and 'wsB' in data:
...
async def main_logic(websocket): # Das ", path" wurde entfernt, um den TypeError zu beheben
...
async def main_async_server():
    ...
    async with websockets.serve(main_logic, "0.0.0.0", 8888):
        ...
        # HÃ¤lt den Server am Laufen, ohne die CPU zu belasten
